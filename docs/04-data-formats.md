...
|---|---|---|---|
| Knowledge graph | N-Triples (`.nt`) | `Normalization/KG/<KG>/<rdf_file>` | `KG`, `rdf_file` |
| Rules | CSV, AMIE-style | `Normalization/KG/<KG>/<rules_file>` | `rules_file` |
| Constraints | Turtle (`.ttl`) with SHACL-SPARQL shapes | `Normalization/Constraints/<constraints_folder>/` | `constraints_folder` |
...
All rule files bundled in `Normalization/KG/<KG>/` use the lowercase names. To convert a file from another
...
df.to_csv("Normalization/KG/<KG>/my_rules.csv", index=False)
```
